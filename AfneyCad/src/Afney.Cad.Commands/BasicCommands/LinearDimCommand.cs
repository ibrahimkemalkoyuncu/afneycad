...
public class LinearDimCommand : ICadCommand
{
    ...
    public LinearDimCommand(CadDatabase db, TransactionManager tm, double textHeight = 250.0)
    {
        _database   = db;
        _tm         = tm;
    ...
    public void OnPointerPressed(Vector3D point)
    {
        ...
        {
            _p1    = point;
            _ghost = new DimensionEntity(point, point, point, DimensionType.Linear) { TextHeight = _textHeight };
            OnFeedback?.Invoke("DIMLINEAR: İkinci ölçü noktasını seçin.");
        }
        else if (_p2 == null)
        {
            _p2    = point;
            _ghost = new DimensionEntity(_p1.Value, point, point, DimensionType.Linear) { TextHeight = _textHeight };
            OnFeedback?.Invoke("DIMLINEAR: Ölçü çizgisi konumunu belirtin.");
        }
        else
        {
            var dim = new DimensionEntity(_p1.Value, _p2.Value, point, DimensionType.Linear)
            {
