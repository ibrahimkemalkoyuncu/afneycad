...
3. **Manuel Mahal — uç-yakala ölçüm modu (YENİ, AYRI BİR BUTON olarak):** kullanıcı isteği: "duvarın bir ucunu yakalayıp fareyi diğer ucuna götürüp yakalayınca o kenarı hesaba alacak şekilde ölçmeli" — mevcut "Manuel" butonu/modeli AYNEN KALACAK (tüm duvar varlığını tek tıkla seçip greedy zincirleme yapıyor, artık self-intersection korumalı). Kullanıcının KARARI: bu yeni köşe-köşe tıklama modeli MEVCUT "Manuel" butonunun YERİNE DEĞİL, "2. Uç Noktalar" ribbon'una eklenecek YENİ, AYRI bir buton olarak eklenmeli (ör. "Kenar Yakala" / "Uç-Uca") — böylece iki mod da bağımsız var olur, hiçbiri bozulmaz. Sıradaki oturumda: yeni bir `ICadCommand` (ör. `EdgeCaptureMahalCommand`) — köşe-köşe tıklayarak sınırı DOĞRU SIRADA inşa eder (greedy yeniden sıralamaya ihtiyaç duymaz, self-intersection riski yapısal olarak yok), `MainWindow.xaml`'e yeni buton + Click handler eklenir.
