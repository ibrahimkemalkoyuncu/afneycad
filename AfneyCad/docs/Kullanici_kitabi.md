...
### 47. İkinci Tur Kod Kalitesi/Performans Taraması — 4 Paralel Düzeltme Uygulandı
Kullanıcı "kod kalitesinde iyileştirmeler yapılması gereken yerleri tespit et ve performans artışı sağla" dedi. Madde 43'teki mimari denetimin ikinci turu: 3 paralel araştırma ajanı (Mechanical/Application, Presentation/Render, Database/Infrastructure/Geometry) katman katman tarayıp ~23 somut, dosya:satır referanslı bulgu çıkardı. En etkili/güvenli olanlar 4 paralel izole worktree ajanına dağıtılıp tek tek doğrulanarak merge edildi.

**1. UI thread async dönüşümleri + reflection kaldırma (`commit adfa517`):** `OnRecalculatePlumbing`, `OnShow3D` (bina montajı), `OnGenerateHydraulicReport` — üçü de senkron ağır hesaplama ile UI thread'i bloklarken artık `OnRecalculateSystem`'deki (Session #62) `Task.Run`+`IProgress<(int,string)>` deseniyle async. `CadViewport.Input.cs`'teki boru grip sürükleme handler'ı her `MouseMove`'da (saniyede onlarca kez) reflection ile `MainWindow`'un private `_mechanicalKernel` alanını okuyordu — artık `CadViewport.MechanicalKernel` public property'siyle doğrudan enjekte ediliyor, reflection tamamen kalktı.

**2. Mechanical/Application spatial-index entegrasyonu (`commit 7af8638`):** `AdvancedSnapService.FindBestSnap` her mouse-move'da 5 alt-metodun (Intersection/Quadrant/Tangent/Extension/Nearest) her biri kendi `GetEntitiesNear` taramasını yapıyordu (5x tam DB taraması) — artık BİR KEZ hesaplanıp 5 metoda parametre geçiliyor. `AdvancedSelectionService` fence/polygon seçimi artık `SpatialQueryService.QueryVisible` ile önce bounding-box aday kümesi daraltıyor. `ConnectReceptorsService` boruları sistem tipine göre önceden `Dictionary`'ye grupluyor (tam spatial index yerine daha düşük riskli ara adım — bilinçli tercih). `MepCoordinationService`'te duvar filtresi ve obstacle centroid'leri döngü dışına taşındı (önceden pipe sayısı kadar tekrar hesaplanıyordu). `NetworkTopologyAnalysisService.BuildNodeGraph` O(n²) node kümelemesini grid-hash ile 3×3×3 komşu hücre taramasına indirdi (tolerans/tie-break semantiği birebir korunarak). `SpatialQueryService`'teki hiç çağrılmayan `Insert`/`Remove` (dirty-flag+rebuild zaten tek gerçek yol) silindi.

**3. Infrastructure/Geometry düzeltmeleri (`commit b7d816a`):** `RevitIfcMappingService` — `IfcImportService`'in kullandığı Dictionary-tabanlı STEP-id indeksleme deseni buraya da getirildi (önceden her boru/cihaz/kanal dönüşümünde TÜM STEP entity listesi lineer taranıyordu). `NURBSCurve.GetDerivative` artık türev eğrisinin kontrol noktalarını/knot'larını (parametreye bağlı olmadığından) bir kez hesaplayıp cache'liyor, kullanılmayan `FindSpan` çağrısı silindi. `DxfReader`'da INSERT işlerken blok arama artık `Dictionary` ile O(1) (önceden her INSERT için tüm blok listesi taranıyordu). Kesin ölü mock kod (`DwgBinaryReader.cs`/`DwgEntityMapper.cs` — gerçek okuma zaten `DxfReader`+ACadSharp üzerinden yapılıyor, bu ikisi hiçbir yerden çağrılmıyordu, `ParseMockEntities` hep sabit bir sahte LINE döndürüyordu) silindi. `IfcImportService`'in çift-parse'ı (Preview/Import ayrı instance kullandığı doğrulanıp) bilinçli olarak atlandı — cache eklemek etkisiz olurdu.

**4. Render katmanı paint cache (`commit 418fe67`):** `CadViewport.Rendering.cs`'deki `DrawUCSIcon`/`DrawSnapMarker`/`DrawSelectionBox` — Session #59'da origin/grid/crosshair paint'lerinin field'a alındığı desen bu 3 metodu kaçırmıştı, her frame 5-9 `SKPaint` yeniden yaratılıyordu; artık field/dictionary olarak cache'leniyor. `SkiaRenderContext.DrawFilledPolygon` (Hatch) sınıfın geri kalanının kullandığı `_paintCache` mekanizmasını atlayıp her çağrıda yeni paint yaratıyordu — `GetFillPaint` ile aynı cache'e bağlandı. `DrawSpline`'ın parametre tipi `IEnumerable<Vector3D>`→`IReadOnlyList<Vector3D>` değişti — `SplineEntity`'nin zaten cache'lenmiş `Tessellate()` sonucu her frame `.ToList()` ile gereksiz kopyalanıyordu (Session #43'teki cache kazanımının bir kısmı burada kayboluyordu). Görsel çıktı (renk/kalınlık/stil/alpha) birebir korundu, sadece allocation zamanlaması değişti.

**Bilinçli olarak ertelenen (mimari risk):** `Matrix4x4` struct'ı `private double[,] _m` heap array taşıyor — struct kopyalandığında aynı array referansı paylaşılıyor, yani biri indexer ile mutasyona uğrarsa "bağımsız" kopya da değişir (correctness landmine). Kod tabanında şu an bu deseni tetikleyen bir kopyala-sonra-mutasyon KULLANIMI YOK (doğrulandı — tüm kullanımlar taze `new Matrix4x4()` üretip dolduruyor) ama gelecekte biri doğal bir şekilde kopyalayıp değiştirirse sessiz bir hataya dönüşebilir. Düzeltmesi (flat 16-alanlı immutable struct veya gerçek defensive-copy) temel bir geometri tipini etkileyen büyük bir refactor — bu oturumda dokunulmadı, bir sonraki oturuma not düşüldü.

**Tam suite: 474/474**, tam çözüm derlemesi 0 hata, her commit ayrı ayrı build+test ile doğrulandı, regresyon yok.
...
### 58. Derinlemesine Mimari Denetim (Undo/Redo, Test Kapsamı, Güvenlik/Stabilite, Performans) — Tüm Kod Tabanı Genelinde
Kullanıcı "sonrasında derinlemesine bir kalite/mimari denetimi" istedi — madde 43'ün (Session #62 tarzı) 4 paralel araştırma ajanı deseni tekrarlandı, ama bu kez çok daha büyümüş bir kod tabanı (CSG Boolean kernel, D3D11 render, FILLET/CHAMFER, IFC/DXF solid import/export, Polar/OSnap tracking, Sheet Set Manager vb.) genelinde.

**1. Undo/Redo mimarisi — EN KRİTİK BULGU: 13 komut TransactionManager'ı tamamen atlıyor.** `TransactionManager`'ın kendisi sağlam (LinkedList+MaxUndoLevels=200 fix hâlâ kalıcı, ölü `IReversibleAction` ailesi geri sızmamış). Ama araştırma, `PlaceDamperCommand` (en karmaşık: hat böler, 3 entity'ye kadar mutasyon) başta olmak üzere 13 `ICadCommand`'in `_database.AddEntity/RemoveEntity`'yi doğrudan çağırdığını buldu — Ctrl+Z bu komutlar için sessizce hiçbir şey yapmıyordu. `PlaceFixtureCommand`'da bunu "Undoable" diye iddia eden YANLIŞ bir yorum bile vardı. **Tümü düzeltildi** (`commit c198ed8`): PlaceDamperCommand, BlockCommand, InsertCommand, AutoDetectSpacesCommand, InsertBOMTableCommand, LegendGenerateCommand, MahalDefineCommand, MahalInspectCommand, PlaceAirTerminalCommand, PlaceFixtureCommand, PlaceFixtureOnWallCommand, RiserGenerateCommand, SmartLabelCommand — hepsi `LineCommand`/`FilletCommand` ile aynı `CompositeOperation`+`TransactionManager.Submit` desenine geçirildi. **Yan bulgu (daha derin):** `MahalDefineCommand.AnalyzeAndAddFixtures`, tespit ettiği cihazları kullanıcı RoomTagDialog'u iptal etse BİLE kalıcı olarak ekliyordu (mahal'in kendisi dialog onaylanınca ekleniyordu ama cihazlar dialog sonucundan bağımsız, `OnPointerPressed` anında spliniyordu) — `PendingFixtures` listesiyle ertelenip mahal ile tek bir composite'e taşındı.

**2. Test kapsamı — iki kör nokta, ikisi de bu oturumun kendi bulduğu hatalarla aynı sınıf:** FILLET/CHAMFER'ın yeni "teğet noktası çizgi dışına taşıyor" reddi hiç test edilmiyordu; IFC dirsek/T-parça round-trip testi sadece Center/InnerDiameter'ı doğruluyordu (tam da 180° yön inversiyonu hatasının kaçtığı nokta). **İkisi de kapatıldı** (`commit fb3cdc9`, 571→576 test): FilletChamferMathTests'e 5 yeni test (her iki kolda aşma reddi + off-by-one olmadığını kanıtlayan sınır-durum başarısı), IfcFittingImportTests'e IncomingVector/MainDirection assertion'ları eklendi. **Ertelenen (daha büyük iş, ayrı görev):** NFPA13SprinklerService sıfır testli (yangın güvenliği kritik), BOM/cost servisleri (BomService/HvacBomService/SelectionBomService/ArchitecturalBomService) testsiz, GeneralSolidUnion testleri hâlâ nispeten basit senaryolarla sınırlı.

**3. Güvenlik/stabilite — ciddi açık YOK, iyi haber:** IFC STEP parser'da döngüsel referans/sonsuz döngü riski bulunamadı (tek-hop dictionary lookup, recursive graph walk yok); `TaskScheduler.UnobservedTaskException` ve Serilog retention fix'leri hâlâ kalıcı; `CloudBackupService` saf yerel dosya kopyalama (kimlik bilgisi/token yok). Tek gerçek bulgu **düzeltildi**: `IfcImportDialog.Preview_Click`, `Import_Click`'in aksine ağır `AnalyzeFile` çağrısını UI thread'inde senkron çalıştırıyordu — büyük IFC dosyalarında önizleme donması (`commit a22282e`, `Task.Run` deseni uygulandı).

**4. Performans/ölü kod — çoğu bulgu düşük önem çıktı, doğru tasarım korunuyor:** `PathfindingService` hâlâ ölü kod (hiç çağrılmıyor, madde 43'te de aynı tespit). FILLET/CHAMFER/SolidBoolean/PlaceDamper'ın "linear scan" bulgusu gerçek ama düşük önemli — sadece tıklama başına çalışıyor (mouse-move'da değil), tipik bir MEP çizimde milisaniye altı. CSG/Direct3D tessellation ZATEN önbellekleniyor (per-frame yeniden hesaplama yok) — bu bulgu YANLIŞ çıktı, madde 55'teki SplineEntity cache deseniyle aynı pipeline'a bağlı. **Düzeltilen:** `SolidEntity.cs`'nin export/import ve Direct3D bağlantısını "henüz yok" diye yanlış tarif eden eski başlık yorumu (`commit a22282e`). **Ertelenen (maintenance smell, acil değil):** FilletCommand/ChamferCommand'ın SolidBooleanCommandBase'e benzer bir ortak taban sınıfla birleştirilebilecek ~90% kod tekrarı.
